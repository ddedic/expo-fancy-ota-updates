...
2. **npm organization** — Create `@technabit` org at [npmjs.com/org/create](https://www.npmjs.com/org/create) (if not already created)
...
```bash
cd packages/@ddedic/expo-fancy-ota-updates
npm pack --dry-run
```
...
The `--access public` flag is required for scoped packages (`@technabit/*`) the first time.
...
Make sure you're a member of the `@technabit` organization on npm with publish rights.
