...
### Channel Surfing for QA

Let QA switch to the `staging` channel at runtime to preview updates before production:

```tsx
function QAChannelPicker() {
...
::: tip
Channel surfing requires Expo SDK 54+. In DEV mode and simulators the call is skipped and returns a `SwitchChannelResult` with `isSkipped: true`.
:::
