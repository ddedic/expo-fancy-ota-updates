...
import type { OTAConfig } from './schema';
 ...
 */
export async function generateChangelog(config: OTAConfig): Promise<string[]> {
  switch (config.changelog.source) {
    ...
    case 'file':
      ...
      return getFileChangelog(config.changelog.filePath);
    
    case 'manual':
      return getManualChangelog();
    
    case 'custom':
      // Custom generator should be provided via hooks
      throw new Error('Custom changelog generator not implemented. Use hooks.beforePublish to set changelog.');
