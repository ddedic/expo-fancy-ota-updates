...
export function OTAUpdatesProvider({ 
  ...
  const switchChannel = useCallback(async (channel: string): Promise<SwitchChannelResult> => {
    const previousChannel = Updates.channel ?? null;

    if (__DEV__) {
      log('Skipping channel switch in DEV mode');
      return { success: false, previousChannel, newChannel: channel, isSkipped: true, reason: 'DEV mode — channel switching requires a production build' };
    }

    if (!Device.isDevice) {
      log('Skipping channel switch in simulator mode');
      return { success: false, previousChannel, newChannel: channel, isSkipped: true, reason: 'Simulator mode' };
    }

    if (!Updates.isEnabled) {
      log('Skipping channel switch: Updates.isEnabled is false');
      return { success: false, previousChannel, newChannel: channel, isSkipped: true, reason: 'Updates disabled' };
    }

    try {
      setIsSwitchingChannel(true);
      log(`Switching channel from "${previousChannel}" to "${channel}"...`);

      Updates.setUpdateRequestHeadersOverride({ 'expo-channel-name': channel });

      log('Channel header set, checking for updates on new channel...');
      await checkForUpdate();

      log(`Channel switched to "${channel}"`);
      return { success: true, previousChannel, newChannel: channel };
    } catch (error) {
      console.error('[OTA] Error switching channel:', error);
      return { success: false, previousChannel, newChannel: channel, error: error as Error };
    } finally {
